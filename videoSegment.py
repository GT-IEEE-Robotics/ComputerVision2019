...
def videoSegment(vidFrame):
    currentScene = vidFrame
    segmentedMask, redMask, blueMask, greenMask = colorMasks(currentScene) 
    segmentedImage = segmentedMask
...
def videoStream():
    cameraFrame = cv2.VideoCapture(1)
    
    frameRate = 10
    previousUpdate = 0

    while True:
        timeElapsedSinceUpdate = time.time() - previousUpdate
        res, image = cameraFrame.read()

        if timeElapsedSinceUpdate > 1./frameRate:
            previousUpdate = time.time()
            _, currentFrame = cameraFrame.read()
            cv2.imshow("Current Frame", currentFrame)
            cv2.waitKey(1)
            videoSegment(currentFrame)
